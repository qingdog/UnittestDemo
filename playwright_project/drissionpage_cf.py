...
def click_cloudflare_turnstile(tab: MixTab, button: ChromiumElement = None):
    ...
    if button:
        ...
        buttons = tab.eles('css=div.ant-space.ant-space-horizontal.ant-space-align-center.ant-space-gap-row-small.ant-space-gap-col-small')
        if len(buttons) >= 6:
            buttons_text = ""
            for i in range(len(buttons)):
                buttons_text += buttons[i].text + " "
            logger.info(f"{buttons_text}")
        else: logger.warning("疑似不在首页...")
